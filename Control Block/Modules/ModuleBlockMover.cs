...
namespace Control_Block
{
    public class ModuleBlockMover : Module, TechAudio.IModuleAudioProvider
    {
        ...
        public virtual void OnPool() //Creation
        {
            ...
            while (I < PartCount)
            {
                Transform transform = block.transform.GetChild(index);
                if (transform.name.Contains("BlockMover"))
                {
                    if (seenFirst)
                    {
                        parts[I] = transform;
            ...
            block.DetachingEvent.Subscribe(Detatch);
        ...
        {
            if (Heart != ControlBlocksMod.PistonHeart)
            {
                Heart = ControlBlocksMod.PistonHeart;
                return;
            }
            try
            {
        ...
        [Serializable]
        public new class SerialData : Module.SerialData<ModuleBlockMover.SerialData>
        {
        ...
        {
            playerSyncByte = GlobalPlayerSyncByte;
            Heart = Control_Block.ControlBlocksMod.PistonHeart;
            Dirty = true;
        ...
        internal void Detatch()
        {
            ...
            block.tank.GetComponent<TechComponentInjector.TechPhysicsReset>().Unsubscribe(PreResetPhysics, PostResetPhysics);
